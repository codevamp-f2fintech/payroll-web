...
export const fetchAttendances = createAsyncThunk(
  ...
  async ({ month, year, page, limit, keyword, }: { month: number, year: number, page: number, limit: number, keyword: string }, { getState }) => {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_APP_URL}/attendance/get?month=${month}&year=${year}&page=${page}&limit=${limit}&keyword=${keyword}`,
      {
        method: 'GET',
        headers: {
...
export const fetchEmployeeAttendances = createAsyncThunk(
  ...
  async ({ employeeId, year, month }: { employeeId: string; year: string; month: string }) => {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_APP_URL}/attendance/employee/${employeeId}/${year}/${month}`, {
      method: 'GET',
      headers: {
